/**
 * translate-words-ja.mjs
 *
 * Translates all word entries in word-entries.json to Japanese.
 * Adds meaning_ja and example_ja fields using Claude API in batches of 50.
 * Saves progress every 100 entries so it can resume if interrupted.
 *
 * Usage:
 *   ANTHROPIC_API_KEY=sk-... node scripts/translate-words-ja.mjs
 *
 * To force re-translate entries that already have Japanese fields:
 *   ANTHROPIC_API_KEY=sk-... node scripts/translate-words-ja.mjs --force
 */
...
/**
 * Translate a batch of word entries to Japanese using Claude.
 * @param {Array<{id: string, meaning_ko: string, example_en: string, example_ko: string}>} batch
 * @returns {Promise<Record<string, {meaning_ja: string, example_ja: string}>>}
 */
async function translateBatch(batch) {
  const systemPrompt = `You are a professional English-Japanese translator for a language learning app.
You will be given a list of English words with their Korean meanings and example sentences.
Translate each word's meaning and example sentence into natural, accurate Japanese.

Rules:
- meaning_ja should be comma-separated Japanese translations, similar in style to the Korean meaning_ko field.
- example_ja should be a natural Japanese translation of the English example sentence.
- Keep translations concise and natural -- this is for language learners.
- Output ONLY valid JSON, no markdown fences, no explanation.
- Output format: { "word_id": { "meaning_ja": "...", "example_ja": "..." }, ... }`;
  ...
  const userPrompt = `Translate these ${items.length} English words to Japanese.\n\n${JSON.stringify(items, null, 2)}`;
...
async function main() {
  ...
  for (let i = 0; i < keysToTranslate.length; i += BATCH_SIZE) {
    ...
    for (const key of batchKeys) {
      ...
      if (translation && translation.meaning_ja && translation.example_ja) {
        entries[key].meaning_ja = translation.meaning_ja;
        entries[key].example_ja = translation.example_ja;
        batchTranslated++;
      } else {
