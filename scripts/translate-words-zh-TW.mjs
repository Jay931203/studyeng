/**
 * translate-words-zh-TW.mjs
 *
 * Translates all word entries in word-entries.json to Traditional Chinese.
 * Adds meaning_zhTW and example_zhTW fields using Claude API in batches of 50.
 * Saves progress every 100 entries so it can resume if interrupted.
 *
 * Usage:
 *   ANTHROPIC_API_KEY=sk-... node scripts/translate-words-zh-TW.mjs
 *
 * To force re-translate entries that already have Traditional Chinese fields:
 *   ANTHROPIC_API_KEY=sk-... node scripts/translate-words-zh-TW.mjs --force
 */
...
/**
 * Translate a batch of word entries to Traditional Chinese using Claude.
 * @param {Array<{id: string, meaning_ko: string, example_en: string, example_ko: string}>} batch
 * @returns {Promise<Record<string, {meaning_zhTW: string, example_zhTW: string}>>}
 */
async function translateBatch(batch) {
  const systemPrompt = `You are a professional English-Traditional Chinese translator for a language learning app.
You will be given a list of English words with their Korean meanings and example sentences.
Translate each word's meaning and example sentence into natural, accurate Traditional Chinese (繁體中文, as used in Taiwan).

Rules:
- meaning_zhTW should be comma-separated Traditional Chinese translations, similar in style to the Korean meaning_ko field.
- example_zhTW should be a natural Traditional Chinese translation of the English example sentence.
...
- Output format: { "word_id": { "meaning_zhTW": "...", "example_zhTW": "..." }, ... }`;
  ...
  const userPrompt = `Translate these ${items.length} English words to Traditional Chinese.\n\n${JSON.stringify(items, null, 2)}`;
...
async function main() {
  ...
  for (let i = 0; i < keysToTranslate.length; i += BATCH_SIZE) {
    ...
    for (const key of batchKeys) {
      ...
      if (translation && translation.meaning_zhTW && translation.example_zhTW) {
        entries[key].meaning_zhTW = translation.meaning_zhTW;
        entries[key].example_zhTW = translation.example_zhTW;
        batchTranslated++;
      } else {
