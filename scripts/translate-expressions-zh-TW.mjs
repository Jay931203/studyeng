...
/**
 * translate-expressions-zh-TW.mjs
 *
 * Translates expression entries' Korean meanings to Traditional Chinese using Claude API.
 ...
 *
 * Usage:
 *   ANTHROPIC_API_KEY=sk-... node scripts/translate-expressions-zh-TW.mjs
 *
 * Optional flags:
...
const PROGRESS_PATH = path.resolve(__dirname, '../src/data/.translate-zhTW-progress.json');
...
/**
 * Call Claude API to translate a batch of expressions to Traditional Chinese.
 * @param {{ id: string; canonical: string; meaning_ko: string }[]} batch
 * @returns {Promise<Record<string, string>>} Map of id -> meaning_zhTW
 */
async function translateBatch(batch) {
  ...
  const systemPrompt = [
    'You are a professional translator specializing in English, Korean, and Traditional Chinese.',
    'Translate these English expressions\' meanings to Traditional Chinese (繁體中文, as used in Taiwan).',
    'Each expression has an English canonical form and Korean meaning.',
    'Provide natural, concise Traditional Chinese translations (similar style to the Korean ones).',
    '',
    'RULES:',
    '- Output ONLY valid JSON: an array of objects with "id" and "meaning_zhTW" fields.',
    '- Keep the same "id" values from the input.',
    '- The Traditional Chinese meaning should be a natural dictionary-style definition, not a literal word-for-word translation.',
    '- Use Taiwan-standard Traditional Chinese (繁體中文), not Simplified Chinese.',
  ...
  /** @type {{ id: string; meaning_zhTW: string }[]} */
  let parsed;
  try {
  ...
  for (const item of parsed) {
    if (item.id && item.meaning_zhTW) {
      map[item.id] = item.meaning_zhTW;
    }
  }
  return map;
}
...
async function main() {
  ...
  const needsTranslation = allIds.filter((id) => {
    ...
    // Skip if already has meaning_zhTW
    if (entry.meaning_zhTW) return false;
  ...
  const alreadyHaveZhTW = allIds.filter((id) => entries[id].meaning_zhTW).length;
  const noKo = allIds.filter((id) => !entries[id].meaning_ko).length;

  console.log(`[INFO] Already have meaning_zhTW: ${alreadyHaveZhTW}`);
  ...
  if (needsTranslation.length === 0) {
    console.log('[INFO] Nothing to translate. All entries already have meaning_zhTW.');
  ...
  for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
    ...
    try {
      ...
      for (const [id, meaningZhTW] of Object.entries(translations)) {
        if (entries[id]) {
          entries[id].meaning_zhTW = meaningZhTW;
          alreadyDone.add(id);
          translated++;
          sinceLastSave++;
        }
  ...
  console.log(`Translated (zhTW):  ${translated}`);
  console.log(`Failed:             ${failed}`);
  console.log(`Skipped (no ko):    ${noKo}`);
  console.log(`Already had zhTW:   ${alreadyHaveZhTW}`);
  console.log('[DONE]');
}
