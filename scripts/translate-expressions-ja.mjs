...
/**
 * translate-expressions-ja.mjs
 *
 * Translates expression entries' Korean meanings to Japanese using Claude API.
 ...
 *
 * Usage:
 *   ANTHROPIC_API_KEY=sk-... node scripts/translate-expressions-ja.mjs
 *
 * Optional flags:
...
const PROGRESS_PATH = path.resolve(__dirname, '../src/data/.translate-ja-progress.json');
...
/**
 * Call Claude API to translate a batch of expressions to Japanese.
 * @param {{ id: string; canonical: string; meaning_ko: string }[]} batch
 * @returns {Promise<Record<string, string>>} Map of id -> meaning_ja
 */
async function translateBatch(batch) {
  ...
  const systemPrompt = [
    'You are a professional translator specializing in English, Korean, and Japanese.',
    'Translate these English expressions\' meanings to Japanese.',
    'Each expression has an English canonical form and Korean meaning.',
    'Provide natural, concise Japanese translations (similar style to the Korean ones).',
    '',
    'RULES:',
    '- Output ONLY valid JSON: an array of objects with "id" and "meaning_ja" fields.',
    '- Keep the same "id" values from the input.',
    '- The Japanese meaning should be a natural dictionary-style definition, not a literal word-for-word translation.',
    '- Use plain form (dictionary form) for verbs, e.g. "朝食を作る" not "朝食を作ります".',
  ...
  /** @type {{ id: string; meaning_ja: string }[]} */
  let parsed;
  try {
  ...
  for (const item of parsed) {
    if (item.id && item.meaning_ja) {
      map[item.id] = item.meaning_ja;
    }
  }
  return map;
}
...
async function main() {
  ...
  const needsTranslation = allIds.filter((id) => {
    ...
    // Skip if already has meaning_ja
    if (entry.meaning_ja) return false;
  ...
  const alreadyHaveJa = allIds.filter((id) => entries[id].meaning_ja).length;
  const noKo = allIds.filter((id) => !entries[id].meaning_ko).length;

  console.log(`[INFO] Already have meaning_ja: ${alreadyHaveJa}`);
  ...
  if (needsTranslation.length === 0) {
    console.log('[INFO] Nothing to translate. All entries already have meaning_ja.');
  ...
  for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
    ...
    try {
      ...
      for (const [id, meaningJa] of Object.entries(translations)) {
        if (entries[id]) {
          entries[id].meaning_ja = meaningJa;
          alreadyDone.add(id);
          translated++;
          sinceLastSave++;
        }
  ...
  console.log(`Translated (ja):  ${translated}`);
  console.log(`Failed:           ${failed}`);
  console.log(`Skipped (no ko):  ${noKo}`);
  console.log(`Already had ja:   ${alreadyHaveJa}`);
  console.log('[DONE]');
}
