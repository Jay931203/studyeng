/**
 * Batch translate all transcript files to add Japanese translations.
 *
 * Usage:
 *   ANTHROPIC_API_KEY=sk-... node scripts/translate-transcripts-ja.mjs
 *
 * Features:
 *   - Scans public/transcripts/*.json for all transcript files
 *   - Skips files that already have `ja` field in every segment
 *   - Batches large files (>50 segments) into chunks of 50
 *   - Saves progress to output/ja-translation-progress.json for restart
...
const PROGRESS_FILE = path.resolve(__dirname, '../output/ja-translation-progress.json');
...
/**
 * Call the Claude API to translate English segments to Japanese.
 * @param {string[]} englishTexts - Array of English text strings
 * @returns {Promise<string[]>} Array of Japanese translations in the same order
 */
async function translateBatch(englishTexts) {
  const prompt = [
    'Translate these English subtitle segments to natural Japanese.',
    'Keep translations concise and conversational, matching the tone of the original.',
    'Return a JSON array of just the Japanese translations in the same order.',
    '',
    'Segments:',
...
/**
 * Check whether a transcript file already has Japanese translations on all segments.
 * @param {Array<{en: string, ko?: string, ja?: string}>} segments
 * @returns {boolean}
 */
function hasJapanese(segments) {
  return segments.every(
    (seg) => seg.ja !== undefined && seg.ja !== null
  );
}

/**
 * Translate a single transcript file, adding `ja` fields.
...
async function translateFile(filePath, segments) {
  ...
  for (let i = 0; i < segments.length; i++) {
    if (segments[i].ja === undefined || segments[i].ja === null) {
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    for (let j = 0; j < batch.length; j++) {
      segments[batch[j].index].ja = translations[j];
    }
  }
