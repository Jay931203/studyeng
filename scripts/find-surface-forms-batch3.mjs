...
function findSurfaceForm(exprId, sentence) {
  ...
  const sentWords = sentence.match(/[\w']+/g) || [];
  const sentPositions = [];

  // Track original positions and text
  let pos = 0;
  const wordMatches = [];
  const regex = /[\w']+/g;
  let m;
  ...
  for (let start = 0; start <= wordMatches.length - n; start++) {
    // Try to match exprTokens in order, with possible gaps (for inserted objects)
...
function normalizeSentence(sentence) {
  // Remove possessive 's so "friend's" matches "friend"
  return sentence.replace(/'\s*s\b/gi, '').replace(/[\u2018\u2019]/g, "'");
}

// Try finding with normalized sentence (possessives, etc.)
function findSurfaceFormExtended(exprId, sentence) {
  ...
  // The regex /[\w']+/ won't capture hyphens, try replacing - with space
  if (exprId.includes('-')) {
  ...
  if (normalized !== sentence) {
    ...
    if (result) {
      // Map back: find corresponding span in original sentence
      // Simple approach: find first word of result in original sentence
      const firstWord = result.split(/\s+/)[0];
      const origRe = new RegExp(firstWord.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
      const origMatch = sentence.match(origRe);
      if (origMatch) {
        // Extract up to result length, stopping at possessive
        const startIdx = sentence.toLowerCase().indexOf(origMatch[0].toLowerCase());
        // Find the last word of result in original
...
// Main
const input = JSON.parse(readFileSync('C:/tmp/sf-batch-3.json', 'utf-8'));
let found = 0;
let notFound = 0;
...
writeFileSync(
  'C:/Users/hyunj/studyeng/src/data/match-results-v3/batch-3.json',
...
const nullCases = output.filter(m => m.surfaceForm === null).slice(0, 10);
console.log('\nSample null cases:');
