/**
 * Batch translate all transcript files to add Traditional Chinese translations.
 *
 * Usage:
 *   ANTHROPIC_API_KEY=sk-... node scripts/translate-transcripts-zh-TW.mjs
 *
 * Features:
 *   - Scans public/transcripts/*.json for all transcript files
 *   - Skips files that already have `zhTW` field in every segment
 *   - Batches large files (>50 segments) into chunks of 50
 *   - Saves progress to output/zhTW-translation-progress.json for restart
...
const PROGRESS_FILE = path.resolve(__dirname, '../output/zhTW-translation-progress.json');
...
/**
 * Call the Claude API to translate English segments to Traditional Chinese.
 * @param {string[]} englishTexts - Array of English text strings
 * @returns {Promise<string[]>} Array of Traditional Chinese translations in the same order
 */
async function translateBatch(englishTexts) {
  const prompt = [
    'Translate these English subtitle segments to natural Traditional Chinese (繁體中文, as used in Taiwan).',
    'Keep translations concise and conversational, matching the tone of the original.',
    'Use Taiwan-standard Traditional Chinese, not Simplified Chinese.',
    'Return a JSON array of just the Traditional Chinese translations in the same order.',
    '',
    'Segments:',
...
/**
 * Check whether a transcript file already has Traditional Chinese translations on all segments.
 * @param {Array<{en: string, ko?: string, zhTW?: string}>} segments
 * @returns {boolean}
 */
function hasZhTW(segments) {
  return segments.every(
    (seg) => seg.zhTW !== undefined && seg.zhTW !== null
  );
}

/**
 * Translate a single transcript file, adding `zhTW` fields.
...
async function translateFile(filePath, segments) {
  ...
  for (let i = 0; i < segments.length; i++) {
    if (segments[i].zhTW === undefined || segments[i].zhTW === null) {
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    for (let j = 0; j < batch.length; j++) {
      segments[batch[j].index].zhTW = translations[j];
    }
  }
