/**
 * Surface Form Finder - Batch 5
 * Builds input from match-results/batch-5.json + transcript-fix-batches/batch-5.json,
 * then finds the exact surface form of each exprId in the `en` sentence.
 */
import fs from 'fs';
...
function getVariants(base) {
  ...
  if (!irreg.length || !IRREGULARS[base]) {
...
function findSurfaceForm(exprId, sentence) {
  ...
  // Strategy 2: allow 1-3 filler words between tokens (object insertion)
...
// ── Build input from match-results + transcript-fix-batches ──────────────────
const matchPath = 'C:/Users/hyunj/studyeng/src/data/match-results/batch-5.json';
const transFixPath = 'C:/Users/hyunj/studyeng/src/data/transcript-fix-batches/batch-5.json';
const outputDir = 'C:/Users/hyunj/studyeng/src/data/match-results-v3';
const outputPath = path.join(outputDir, 'batch-5.json');
...
console.log(`Built ${input.length} input records from match-results/batch-5.json`);
